 ...
 */ export async function detectContentType(buffer, skipMetadata, concurrency) {
...
export async function fetchExternalImage(href, dangerouslyAllowLocalIP, maximumResponseBody, count = 3) {
    if (!dangerouslyAllowLocalIP) {
        ...
        if (privateIps.length > 0) {
            Log.error('upstream image', href, 'resolved to private ip', JSON.stringify(privateIps));
...
export async function imageOptimizer(imageUpstream, paramsResult, nextConfig, opts) {
    ...
    if (upstreamType.startsWith('image/svg') && !nextConfig.images.dangerouslyAllowSVG) {
        if (!opts.silent) {
            Log.error(`The requested resource "${href}" has type "${upstreamType}" but dangerouslyAllowSVG is disabled. Consider adding the "unoptimized" property to the <Image>.`);
        }
    ...
    } else if (getExtension(upstreamType) && upstreamType !== WEBP && upstreamType !== AVIF) {
        contentType = upstreamType;
    } else {
        contentType = JPEG;
    }
