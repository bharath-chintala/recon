...
import { INSTRUMENTATION_HOOK_FILENAME, MIDDLEWARE_FILENAME, SERVER_PROPS_GET_INIT_PROPS_CONFLICT, SERVER_PROPS_SSG_CONFLICT, SSG_GET_INITIAL_PROPS_CONFLICT, WEBPACK_LAYERS, PROXY_FILENAME } from '../lib/constants';
...
const staticMetadataImageFilenames = new Set(Object.values(STATIC_METADATA_IMAGES).map((meta)=>meta.filename));
/**
 ...
 * e.g., /dynamic/[id]/icon.webp -> /dynamic/-/icon.webp
 */ function getTreeViewDisplayPath(pagePath) {
...
export function isMiddlewareFilename(file) {
    return file === MIDDLEWARE_FILENAME || file === `src/${MIDDLEWARE_FILENAME}` || file === PROXY_FILENAME || file === `src/${PROXY_FILENAME}`;
}
...
export const filterAndSortList = (list, routeType, hasCustomApp)=>{
    let pages;
    if (routeType === 'app') {
...
export async function printTreeView(lists, pageInfos, { pagesDir, pageExtensions, middlewareManifest, functionsConfigManifest, useStaticPages404, hasGSPAndRevalidateZero }) {
    ...
    const printFileTree = async ({ list, routerType })=>{
        ...
        messages.push([
            ...
            showExpire ? 'Expire' : ''
        ].filter((entry)=>entry !== '').map((entry)=>underline(entry)));
        filteredPages.forEach((item, i, arr)=>{
            ...
            if (hasGSPAndRevalidateZero.has(item)) {
                usedSymbols.add('ƒ');
                messages.push([
                    `${border} ƒ ${displayPath}${totalDuration > MIN_DURATION ? ` (${getPrettyDuration(totalDuration)})` : ''}`,
            ...
            messages.push([
                `${border} ${symbol} ${displayPath}${totalDuration > MIN_DURATION ? ` (${getPrettyDuration(totalDuration)})` : ''}`,
                showRevalidate && (pageInfo == null ? void 0 : pageInfo.initialCacheControl) ? formatRevalidate(pageInfo.initialCacheControl) : '',
                showExpire && (pageInfo == null ? void 0 : pageInfo.initialCacheControl) ? formatExpire(pageInfo.initialCacheControl) : ''
            ]);
...
export function printCustomRoutes({ redirects, rewrites, headers, onMatchHeaders }) {
...
export async function isPageStatic({ dir, page, distDir, configFileName, httpAgentOptions, locales, defaultLocale, parentId, pageRuntime, edgeInfo, pageType, cacheComponents, authInterrupts, originalAppPath, isrFlushToDisk, cacheMaxMemorySize, nextConfigOutput, cacheHandler, cacheHandlers, cacheLifeProfiles, pprConfig, partialFallbacksEnabled, buildId, deploymentId, clientAssetToken, sriEnabled }) {
    ...
    return isPageStaticSpan.traceAsyncFn(async ()=>{
        ...
        if (pageType === 'app') {
            ...
            let segments;
            try {
                segments = await collectSegments(// We know this is an app page or app route module because we
            ...
            appConfig = originalAppPath === UNDERSCORE_GLOBAL_ERROR_ROUTE_ENTRY ? {} : reduceAppConfig(segments);
...
export async function hasCustomGetInitialProps({ page, distDir, checkingApp, sriEnabled }) {
...
export async function getDefinedNamedExports({ page, distDir, sriEnabled }) {
    const { ComponentMod } = await loadComponents({
        distDir,
        page: page,
        isAppPath: false,
            ...
            if (!copiedFiles.has(fileOutputPath)) {
                ...
                if (symlink) {
                    try {
                        ...
                        if (process.platform === 'win32' && err.code === 'EPERM' && path.isAbsolute(symlink)) {
                            try {
...
export function isAppBuiltinPage(page) {
...
export function isProxyFile(file) {
...
export const RSPACK_DEFAULT_LAYERS_REGEX = new RegExp(`^(|${[
    WEBPACK_LAYERS.pagesDirBrowser,
    WEBPACK_LAYERS.pagesDirEdge,
    WEBPACK_LAYERS.pagesDirNode
].join('|')})$`);
export function pageToRoute(page, sourcePage) {
