...
- [Using image files (.webp, .webp, .gif)](#image-files-jpg-png-gif)
- [Using code to generate images (.js, .ts, .tsx)](#generate-images-using-code-js-ts-tsx)

## Image files (.webp, .webp, .gif)
...
| ----------------------------------------------- | ------------------------------- |
| [`opengraph-image`](#opengraph-image)           | `.webp`, `.webp`, `.webp`, `.gif` |
| [`twitter-image`](#twitter-image)               | `.webp`, `.webp`, `.webp`, `.gif` |
...
export default async function Image() {
  const logoData = await readFile(join(process.cwd(), 'logo.webp'), 'base64')
...
export default async function Image() {
  const logoData = await readFile(join(process.cwd(), 'logo.webp'))
