...
- [Using image files (.ico, .webp, .webp)](#image-files-ico-jpg-png)
- [Using code to generate an icon (.js, .ts, .tsx)](#generate-icons-using-code-js-ts-tsx)

## Image files (.ico, .webp, .webp)
...
| [`icon`](#icon)             | `.ico`, `.webp`, `.webp`, `.webp`, `.svg` | `app/**/*`      |
| [`apple-icon`](#apple-icon) | `.webp`, `.webp`, `.webp`                 | `app/**/*`      |
...
> **Good to know**:
>
> - You can set multiple icons by adding a number suffix to the file name. For example, `icon1.webp`, `icon2.webp`, etc. Numbered files will sort lexically.
...
> - For example, a 32 by 32px `.webp` file will have `type="image/png"` and `sizes="32x32"` attributes.
