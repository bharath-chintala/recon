...
For example, the file `public/avatars/me.webp` can be viewed by visiting the `/avatars/me.webp` path. The code to display that image might look like:

```jsx filename="avatar.js"
import Image from 'next/image'

export function Avatar({ id, alt }) {
  return <Image src={`/avatars/${id}.webp`} alt={alt} width="64" height="64" />
}
