...
5. Any [metadata files](/docs/app/getting-started/metadata-and-og-images#file-based-metadata)
   such as `favicon.ico`, `icon.webp`, `robots.txt` are automatically added to the application
...
import image from './img.webp' // `image` will be '/assets/img.2d8efhg.webp' in production
...
Alternatively, you can reference the public URL for the image asset based on the filename. For example, `public/logo.webp` will serve the image at `/logo.webp` for your application, which would be the `src` value.
