 ...
 *
 * @example
 * const { data, info } = await sharp('my-image.webp')
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.chromaSubsampling)) {
      if (is.string(options.chromaSubsampling) && is.inArray(options.chromaSubsampling, ['4:2:0', '4:4:4'])) {
        this.options.webpChromaSubsampling = options.chromaSubsampling;
      } else {
    ...
    if (is.defined(options.mozjpeg)) {
      if (is.bool(options.mozjpeg)) {
        if (options.mozjpeg) {
          this.options.webpTrellisQuantisation = true;
          this.options.webpOvershootDeringing = true;
          this.options.webpOptimiseScans = true;
          this.options.webpProgressive = true;
          this.options.webpQuantisationTable = 3;
        }
      } else {
    ...
    if (is.defined(quantisationTable)) {
      if (is.integer(quantisationTable) && is.inRange(quantisationTable, 0, 8)) {
        this.options.webpQuantisationTable = quantisationTable;
      } else {
 ...
 * const data = await sharp('input.webp')
 *   .ensureAlpha()
 *   .extractChannel(3)
 *   .toColourspace('b-w')
 *   .raw({ depth: 'ushort' })
